...
exports.findAll = async (req, res) => {
    ...
    try {
        ...
        // LIKE QUERY
        if(!!param.search){
            let colObj= ['name', 'phone_no', 'email', 'insurance_expiry_date', 'address', 'suburb', 'postal_code', 'emergency_contact_name', 'emergency_phone_no', 'notes','travel_distance']
       ...
       let totalRecords =  await Cleaner.count({
            ...
            if (totalRecords > 0) {
                    Cleaner.findAll({ attributes : ['hashcode', 'name', 'phone_no', 'email', 'contract_url', 'insurance_url', 'email_alert', 'address', 'suburb', 'postal_code', 'emergency_contact_name', 'emergency_phone_no', 'notes', 'job_id', 'travel_distance', 'is_active', 
                    [sequelize.fn('date_format', sequelize.col('cleaner.insurance_expiry_date'), datetime_format), 'insurance_expiry_date'],
                    [sequelize.fn('date_format', sequelize.col('cleaner.created_on'), datetime_format), 'created_date'],
                    [sequelize.fn('date_format', sequelize.col('cleaner.updated_on'), datetime_format), 'updated_date'],
                ...
                include: [
                    {
                        ...
                        attributes : [
                            ...
                            [sequelize.fn('IFNULL', sequelize.col('lead.hashcode'), ''),'hashcode'],
                            [sequelize.fn('IFNULL', sequelize.col('lead.client_name'), ''),'client_name']
                        ]
                    },
                    {
                        model: Staff,
...
exports.findById = (req, res) => {
    ...
    try {
        ...
        Cleaner.findOne({ attributes: ['hashcode', 'name', 'phone_no', 'email', 'contract_url', 'insurance_url', 
        'email_alert', 'address', 'suburb', 'postal_code', 'emergency_contact_name', 'emergency_phone_no', 'notes', 'job_id', 'travel_distance', 'is_active',
            [sequelize.fn('date_format', sequelize.col('cleaner.insurance_expiry_date'), datetime_format), 'insurance_expiry_date'],
            [sequelize.fn('date_format', sequelize.col('cleaner.created_on'), datetime_format), 'created_date'],
            [sequelize.fn('date_format', sequelize.col('cleaner.updated_on'), datetime_format), 'updated_date'],
            [sequelize.fn('CONCAT', sequelize.col('s.fname'),' ' ,sequelize.col('s.lname')), 'created_by'],
...
exports.update = (req, res) => {
    ...
    try { 
        Cleaner.findOne({ where: { hashcode: id }}).then(cleanerData => {
            if(cleanerData != null ){
                Cleaner.count({ where: { 'email': param.email, [Op.not]: {hashcode: id}, deleted_on: null }}).then(async (isEmailExists) => {
                    if (isEmailExists > 0) {
                        return res.send(error('Email already used!'))
                    } else {    
                        if(!!param.contract_file){
                            param.contract_file = await fileUpload(param.contract_file, cleanerData.dataValues.contract_url, 'cleaner_contract_file')
                            if(param.contract_file.valid){
                                param.contract_file = param.contract_file.url
                            } else {
                                console.log(param.contract_file.error)
                        ...
                        if(!!param.insurance_file){
                            param.insurance_file = await fileUpload(param.insurance_file, cleanerData.dataValues.insurance_url, 'cleaner_insurance_file')
                            if(param.insurance_file.valid){
                                param.insurance_file = param.insurance_file.url
                            } else {
                                console.log(param.insurance_file.error)
...
exports.delete = (req, res) => {
    ...
    try {   
        Cleaner.count({where : {hashcode : id}}).then(isCleanerExists => {
